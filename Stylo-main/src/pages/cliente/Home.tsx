          ...
          {appointments.length === 0 ? (
            ...
            <div className="space-y-4">
              {appointments.map((app) => (
                <motion.div key={app.id} layout className="p-5 sm:p-8 rounded-[1.75rem] sm:rounded-[2.5rem] bg-theme-bg border border-theme-secondary/10 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center gap-4 sm:gap-6 min-w-0">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-theme-secondary/10 flex items-center justify-center text-2xl sm:text-3xl shrink-0">{app.shopName.toLowerCase().includes('barber') ? '💈' : '✂️'}</div>
                    <div className="min-w-0">
                      <h4 className="font-black text-lg sm:text-xl text-theme-text truncate">{app.shopName}</h4>
                      ...
                      {app.serviceName && <p className="text-xs text-theme-text/60 mt-1 truncate">{app.serviceName}</p>}
                    </div>
                  </div>
          ...
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {appointments.filter(a => a.status !== 'completed' && a.status !== 'cancelled').map(app => (
                <div key={app.id} className="p-5 sm:p-8 rounded-[1.75rem] sm:rounded-[2.5rem] bg-theme-bg border border-theme-secondary/10 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 group hover:shadow-xl transition-all">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 rounded-[1.5rem] bg-theme-secondary/10 flex items-center justify-center text-3xl shadow-inner group-hover:scale-110 transition-transform">
                      {app.shopName.includes('Barber') ? '💈' : '✂️'}
                    </div>
                    <div>
                      <h4 className="font-black text-xl text-theme-text mb-1">{app.shopName}</h4>
